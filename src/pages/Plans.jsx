...
import { fetchPricingData } from '../lib/api'; // Import fetchPricingData
...
const Plans = () => {
  ...
  const [pricingData, setPricingData] = useState([]); // State for API data
  const [loading, setLoading] = useState(true);
  const [openModal, setOpenModal] = useState(false);

  // Fetch user's country and pricing data on mount
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [countryCode, pricingResponse] = await Promise.all([
          getUserCountry(),
          fetchPricingData()
        ]);
        ...
        if (pricingResponse && pricingResponse.success !== false && pricingResponse.data) {
  ...
  // Convert API plans to UI props with current currency
  const plans = useMemo(() => {
    if (!pricingData || pricingData.length === 0) return [];

    // Sort plans to ensure Basic comes first (if needed, or rely on API order)
    // The current UI expects Basic then Premium. API returns Premium then Basic.
    // Let's sort by plan_id to be safe? Basic is 6, Premium is 5.
    // Actually, let's just find them by name to be explicit and safe.

    const basicPlanData = pricingData.find(p => p.name === 'Basic');
    const premiumPlanData = pricingData.find(p => p.name === 'Premium');

    const displayPlans = [];

    // Helper to get price safely
    const getPrice = (plan, type) => {
      const currencyPricing = plan.pricing[currencyInfo.code] || plan.pricing['USD'];
      if (!currencyPricing) return null;
      return currencyPricing[type];
    };

    if (basicPlanData) {
      const basicPricing = getPrice(basicPlanData, 'yearly');
      displayPlans.push({
        title: basicPlanData.name,
        features: basicPlanData.features.map(f => f.label),
        yearly: {
          price: basicPricing ? basicPricing.price.toFixed(2) : '0.00',
          totalYearly: basicPricing ? basicPricing.price.toFixed(2) : '0.00',
          period: '',
        },
    ...
    if (premiumPlanData) {
      const premiumMonthly = getPrice(premiumPlanData, 'monthly');
      const premiumYearly = getPrice(premiumPlanData, 'yearly');

      displayPlans.push({
        title: premiumPlanData.name,
        features: premiumPlanData.features.map(f => f.label),
        monthly: premiumMonthly ? {
          price: premiumMonthly.price.toFixed(2),
          period: 'per month',
        } : null,
        yearly: premiumYearly ? {
          price: (premiumYearly.price / 12).toFixed(2), // Calculate monthly equivalent for display if needed, or just use price?
          // Wait, the UI previously showed `monthlyEq` (9.92) for yearly option.
          // The new API gives `price` (95) and `original_price` (119.88).
          // Let's calculate monthly equivalent: 95 / 12 = 7.9166 -> 7.92.
          // The API defines `USD.yearly.price` as 95.
